import { CardId, getRoomCode, LabelId, ModuleId, RoomType, ServerSE } from '@mosaiq/terrazzo-common';
...
export const syncModuleLabels = async (moduleId: ModuleId) => {
    ...
    await broadcast({
        ...
        toRoomIds: [getRoomCode(RoomType.DATA, moduleId)],
...
export const syncLabel = async (labelId: LabelId) => {
    ...
    await broadcast({
        ...
        toRoomIds: [getRoomCode(RoomType.DATA, label.boardId)],
        buildPayload: async (userId) => {
